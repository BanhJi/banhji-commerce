...
exports.CalendarVue3 = exports.Calendar = void 0; // @ts-ignore
...
var Calendar = {
  ...
  props: {
    ...
    id: {
      ...
      default: function _default() {
        return kendo_vue_common_1.guid();
      }
    },
  ...
  data: function data() {
    return {
      ...
      currentValue: null,
      cellUID: kendo_vue_common_1.guid(),
      isActive: false,
      oldValue: null,
  ...
  created: function created() {
    ...
    this.$data.currentFocusedDate = utils_1.dateInRange(this.$props.focusedDate || calculatedFocus || utils_1.getToday(), this.currentMin, this.currentMax);
    this.$data.currentValue = value;
    this.$data.currentActiveView = utils_1.viewInRange(models_1.CalendarViewEnum[this.$props.defaultActiveView], models_1.CalendarViewEnum[this.$props.bottomView], models_1.CalendarViewEnum[this.$props.topView]);
    this.$data.currentActiveRangeEnd = utils_2.extractActiveRange(selectedRange, selectedDate);
    this._dates = [];
    ...
    this._dom = new services_1.DOMService();
    this._bus = new services_1.BusViewService(this.handleViewChange);
    this._navigation = new services_1.NavigationService(this._bus);
  ...
  updated: function updated() {
    var stateValue = utils_2.extractDateFromValue(this.currentMin, this.currentMax, this.computedValue());
  ...
  computed: {
    ...
    currentMin: function currentMin() {
      return kendo_date_math_1.getDate(this.$props.min);
    },
    currentMax: function currentMax() {
      return kendo_date_math_1.getDate(this.$props.max);
    }
  },
  methods: {
      ...
      switch (this.$props.mode) {
        ...
        case 'multiple':
          if (Array.isArray(this._selectedMultiple)) {
            ...
            index_1 !== -1 ? result.splice(index_1, 1) : result.push(kendo_date_math_1.cloneDate(event.value));
            value = result.slice();
          } else {
            if (this._selectedDate) {
              value = [kendo_date_math_1.cloneDate(this._selectedDate), kendo_date_math_1.cloneDate(event.value)];
            } else {
              value = [kendo_date_math_1.cloneDate(event.value)];
            }
          }
    ...
    tableKeyDown: function tableKeyDown(event) {
      ...
      if (keyCode === kendo_vue_common_1.Keys.enter) {
        ...
        var candidate = utils_1.dateInRange(this._navigation.move(this._focusedDate, this._navigation.action(event), this.$data.currentActiveView, this._service, event), this.currentMin, this.currentMax);

        if (kendo_date_math_1.isEqualDate(this._focusedDate, candidate)) {
          return;
        }
    ...
    isValueEqual: function isValueEqual(newValue, oldValue) {
      if (newValue instanceof Date && oldValue instanceof Date) {
        return kendo_date_math_1.isEqualDate(newValue, oldValue); // @ts-ignore
      } else if (newValue instanceof Object && oldValue instanceof Object) {
        if (newValue && oldValue && // @ts-ignore
        kendo_date_math_1.isEqualDate(newValue.start, oldValue.start) && kendo_date_math_1.isEqualDate(newValue.end, oldValue.end)) {
          return true;
        } else {
          return false;
        }
  ...
  render: function render(createElement) {
    ...
    var activeView = utils_1.viewInRange(this.$data.currentActiveView, models_1.CalendarViewEnum[this.$props.bottomView], models_1.CalendarViewEnum[this.$props.topView]);
    this._selectedDate = utils_2.extractDateFromValue(this.currentMin, this.currentMax, currentValue);
    this._selectedMultiple = utils_2.extractMultipleFromValue(this.currentMin, this.currentMax, currentValue);
    this._selectedRange = utils_2.extractRangeFromValue(currentValue);
    var calculatedFocus = utils_2.extractFocusedDate(this._selectedDate, this._selectedMultiple, this._selectedRange);
    var currentDateInRange = utils_1.dateInRange(didValueChange && calculatedFocus !== null ? calculatedFocus : currentFocusedDate, this.currentMin, this.currentMax);

    if (currentDateInRange instanceof Date) {
      this._focusedDate = kendo_date_math_1.getDate(currentDateInRange);
    ...
    this._bus.configure(models_1.CalendarViewEnum[this.$props.bottomView], models_1.CalendarViewEnum[this.$props.topView]);
    ...
    var activeDate = kendo_date_math_1.cloneDate(this._dates && this._dates[0] ? this._dates[0] : undefined);
...
exports.CalendarVue3 = CalendarVue3;