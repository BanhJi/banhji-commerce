        ...
        <v-col sm="12" cols="12" class="grayBg px-6">
            <v-card color="white" class="pa-3 no_border" elevation="0">
                <v-row>
                    ...
                    <v-col sm="6" cols="12" class="pt-0">
                        <v-btn to="./connect_card_payment" class="white--text text-capitalize float-right" color="primary">
                            {{$t('new_connection')}}
                        </v-btn>
                    </v-col>
                </v-row>
                                ...
                                <template v-slot:default>
                                    ...
                                    <tbody>
                                        <tr v-for="item in banks"
                                            :key="item.id"
                                        >
                                            <td><v-img width="90" class="borderRadius20" :src="item.logo" /></td>
                                            <td><h4>{{ item.bank }}</h4></td>
                                            <td>{{ item.merchant.id }}</td>
                                            <td>
                                                <span v-if="item.coa">{{ item.coa.account.number }} - {{ item.coa.account.name }}</span>
                                            </td>
                                        </tr>
                                    </tbody>
                                </template>
            ...
            loadPartnerService () {
                ...
                .then((res)=>{
                    ...
                    res.data.forEach(value => {
                        if(value.serviceType.toLowerCase() === BankPartnerType.CARD){
