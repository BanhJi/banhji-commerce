<template>
	<v-card>
		<div class="modal_header">
			<v-card-title>{{ $t("bank_field_mapping") }}</v-card-title>
			<v-icon
...
export default {
	...
	data: () => ({
		...
		maxRows : 8,/* Only 8 fields are available now */
		// Form validation
		valid: true,
		isEdit: false,
		saveMode: "",
	...
	methods: {
		...
		setDefaultData() {
			this.bankTransactionMapping = new BankTransactionMappingModel({
				type: BankMappingType.EXCEL_MAPPING,
			});
		...
		banhjiFieldTmpl(dataItem) {
			let field2 = BankMappingField.EXCEL_MAPPING.find(i=>i.value === dataItem.field2);
		...
		banhjiMappingFieldEditor(container, options) {
			kendo
				...
				.kendoDropDownList({
					...
					height: 400,
					optionLabel: "--- Select ---",
					dataSource: BankMappingField.EXCEL_MAPPING,
				});
		},
		// Add New Row
		addRow() {
		...
		validateForm() {
			...
			ds.data().forEach((value, index) => {
				if(value.field === "" || value.field2 === "") {
					msg += `<p>${i18n.t("bank_field_banhji_field_are_required_at_row")} ${index + 1}</p>`;
