                                ...
                                <template v-slot:default>
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="text-bold text-uppercase text-center">{{$t('wht_rate')}}</th>
                                            <th class="text-bold text-uppercase">{{$t('vat_amount')}}</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                    <tr>
                                        <td>{{$t('wht_no_resident_desc1')}}</td>
                                        <td class="text-center">14%</td>
                                        <td class="text-center">{{ numberFormat(wh1) }}</td>
                                    </tr>
                                    <tr>
                                        <td>{{$t('wht_no_resident_desc2')}}</td>
                                        <td class="text-center">14%</td>
                                        <td class="text-center">{{ numberFormat(wh2) }}</td>
                                    </tr>
                                    <tr>
                                        <td>{{$t('wht_no_resident_desc3')}}</td>
                                        <td class="text-center">14%</td>
                                        <td class="text-center">{{ numberFormat(wh3) }}</td>
                                    </tr>
                                    <tr>
                                        <td>{{$t('wht_no_resident_desc4')}}</td>
                                        <td class="text-center">14%</td>
                                        <td class="text-center">{{ numberFormat(wh4) }}</td>
                                    </tr>
                                    <tr>
                                        <td>{{$t('service')}}</td>
                                        <td class="text-center">14%</td>
                                        <td class="text-center">{{ numberFormat(wh5) }}</td>
                                    </tr>
                                    <tr>
                                        <td colspan="2" class="text-right text-uppercase text-bold">{{ $t('total') }}</td>
                                        <td class="primary white--text text-center">{{ numberFormat(totalWh) }}</td>
                                    </tr>
                                    </tbody>
                                </template>
                            </v-simple-table>
...
export default {
    ...
    methods: {
            ...
            new Promise(resolve => {
                setTimeout(() => {
                    ...
                    for (let i = 0; i < this.withholdings.length; i++) {
                        ...
                        if (typeProduct.code === 7) {
                            amount1 += this.withholdings[i].totalVAT
                        } else if (typeProduct.code === 8) {
                            amount2 += this.withholdings[i].totalVAT
                        } else if (typeProduct.code === 9) {
                            amount3 += this.withholdings[i].totalVAT
                        } else if (typeProduct.code === 10) {
                            amount4 += this.withholdings[i].totalVAT
                        }
                    }
                    totalAmount1 = (amount1 + amount2 + amount3 + amount4)
        ...
        onUpdate() {
            this.$emit('getAmountNonResident', this.totalWh)
        },
        numberFormat(value) {
