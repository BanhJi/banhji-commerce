<template>
  <v-row>
    ...
    <v-col sm="2" cols="12" class="pt-0">
      <v-dialog v-model="dialogM3" max-width="600px" eager>
...
export default {
  ...
  methods: {
    ...
    goEdit(e) {
      e.preventDefault();
      let grid = kendo.jQuery("#gridCategory").data("kendoGrid");
      ...
      new Promise((resolve) => {
        setTimeout(() => {
          ...
          categoryHandler.get().then((res) => {
            this.showLoading = false;
            this.categoryList = res.filter((o) => o.type.name === "Service");
          });
        }, 300);
      });
    },
            ...
            .then((response) => {
              if (response.data.statusCode === 201) {
                ...
                if (index < 0) {
                  ...
                  const grid = kendo.jQuery("#gridCategory").data("kendoGrid");
                  grid.refresh();
                } else {
