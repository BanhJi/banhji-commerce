    ...
    <v-row>
        <v-col sm="10" cols="12" class="pt-0">
          ...
          <v-col sm="2" cols="12" class="pt-0">
            <v-dialog v-model="dialogM2" max-width="500px" eager>
                    ...
                    <v-card-text class="modal_text_content">
                        <v-form
                            ...
                            <v-row>
                                ...
                                <v-col class="pt-0">
                                    <h2 class="mb-0 font_20 width_100">{{ $t("default_image") }}</h2>
                                    <v-row>
        ...
        <v-col sm="12" cols="12">
            <template>
                ...
                <kendo-datasource ref="gridGroupDS"
                                  :data="groups"/>
                <kendo-grid id="gridGroup" class="grid-function"
    ...
    methods: {
        ...
        goEdit(e) {
            ...
            if (dataItem.img.hasOwnProperty('default')) {
                this.src = 'https://s3-ap-southeast-1.amazonaws.com/images.banhji/' + dataItem.img.default.bigThumb
            } else {
                this.src = ''
            }
            this.dialogM2 = true
        ...
        async onSaveClose() {
            ...
            new Promise(resolve => {
                setTimeout(() => {
                    ...
                    groupHandler.create(data).then(response => {
            ...
            new Promise(resolve => {
                setTimeout(() => {
                    ...
                    categoryHandler.get().then(res => {
                        ...
                        this.categories = res.filter(o => o.type.name === 'Service')
                        this.loading = false 
                        // this.categories = res
                    })
                }, 300);
            ...
            new Promise(resolve => {
                setTimeout(() => {
                    ...
                    groupHandler.get().then(res => {
                        this.showLoading = false
                        this.groups = res.filter(o => o.category.type.name === 'Service')
                        // res.forEach(i => {
                        //     if (i.category.hasOwnProperty('type')) {
                        //         if (i.category.type.name === "Service") {
                        //             this.groups.push(i)
                        //         }
                        //     }
                        //     this.groups = res.filter(o => o.type.name === 'Service')
                        // })
                        // this.groups = res
                    })
                }, 300);
