				...
				<v-row>
					<v-col sm="9" cols="12" class="py-0">
						<h3 class="text-left mb-1 font_20 line_24">
							{{ $t("budget_performance_report_project") }}
						</h3>
						<p class="mb-0">
							{{ $t("budget_performance_report_project_desc") }}
						</p>
					</v-col>
								...
								<template>
									<tbody>
										...
										<tr>
											...
											<td class="" v-if="summary.project">
												{{ summary.project.name }}
											</td>
										</tr>
										<tr>
											<td class="">{{ $t("fiscal_year") }}</td>
											<td class="" v-if="summary.fiscal_year">
												{{
													Helper.formatFiscalDate(summary.fiscal_year.start_date)
												}}
											</td>
										</tr>
									</tbody>
								...
								<template>
									<tbody>
										<tr>
											...
											<td class="" v-if="summary.created_date">
												{{ Helper.dateFormat(summary.created_date) }}
											</td>
										</tr>
										<tr>
											<td class="">{{ $t("last_edited_on") }}</td>
											<td class="">{{ summary.modified_date }}</td>
										</tr>
										<tr>
											<td class="">{{ $t("last_edited_by") }}</td>
											<td class="">
												{{
													summary.modified_by.first_name === ""
														? summary.created_by.first_name +
															" " +
															summary.created_by.last_name
														: summary.modified_by.first_name +
															" " +
															summary.modified_by.last_name
												}}
											</td>
										</tr>
									</tbody>
...
export default {
	...
	methods:{
		...
		bindData () {
			...
			/* Information */
			this.budget.project = this.getProjectById(this.budget.project_uuid);
				...
				if(value.parentId !== null){
					...
					if(budgetLine){
						...
						if(ownChild && budgetLine.balance !== 0){
							...
							lineOwnChild.uuid += value.type_code;
