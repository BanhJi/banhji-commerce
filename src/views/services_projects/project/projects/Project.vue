            ...
            <v-form ref="form" v-model="valid" lazy-validation>
              ...
              <v-card outlined dense class="px-3 no_border mb-3" color="white">
                ...
                <v-row>
                  ...
                  <v-col sm="12" cols="12">
                    <kendo-datasource ref="customersDSProject"
                      :change="dataSourceChanged"
                      :data="customerList"/>
                    <kendo-grid id="gridCustomers" class="grid-function"
                      :data-source-ref="'customersDSProject'"
                      :editable="true"
                      ...
                      <kendo-grid-column
                        ...
                        :width="70"
                        :attributes="{style: 'text-align: right'}"
                        :command="[{ iconClass: 'k-icon k-i-trash', text: $t('delete'), click: removeRow }]"
...
export default {
  ...
  methods: {
    ...
    rowNumber(dataItem) {
      let ds = this.$refs.customersDSProject.kendoWidget(),
    ...
    customerDropDownEditor(container, options) {
      ...
      .kendoDropDownList({
        ...
        dataSource: new kendo.data.DataSource({
          ...
          transport: {
            read: {
              url: customerHandler.search(strFilter),
            },
            dataType: "json",
          },
          schema: {
    ...
    async onSaveClose(isSave) {
      ...
      d1.forEach((value, index) => {
        if(value.customer == undefined || value.customer.id == undefined || value.customer.id  == '' || value.customer.id == null || value.saleOrder == undefined || value.saleOrder.id == undefined || value.saleOrder.id  == '' || value.saleOrder.id == null){
          this.$snotify.error(i18n.t('select_customer_sale_order') + (index + 1))
        }else{
          dataValidate  += 1
        }
      });
      if(d1.length  == dataValidate){
        new Promise((resolve) => {
          setTimeout(() => {
            resolve("resolved");
            let customerDS = this.$refs.customersDSProject.kendoWidget()
            let customerArr = []
            window.console.log("update product",  customerDS.data());
            customerDS.data().forEach((i) => {
                customerArr.push({
                  amount: i.amount || '',
                  customer: i.customer || {},
                  decimalFormat: i.decimalFormat || {},
                  saleOrder: i.saleOrder || {},
                });
            });
            let data = {
