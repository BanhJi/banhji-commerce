        ...
        >
          ...
          <v-tab :key="1">
            <span class="text-capitalize text-left">
              {{ $t("order_by_customers_r") }}
            </span>
          </v-tab>
          <v-tab :key="2">
            <span class="text-capitalize text-left">
              {{ $t("order_by_product_services_r") }}
            </span>
          </v-tab>
          <v-tab :key="4">
            <span class="text-capitalize text-left">
              {{ $t("order_by_product_services_categories_r") }}
            </span>
          </v-tab>
...
export default {
  ...
  data: () => ({
    isHide: false,
    active_tab_in: data.order_report_tab_sub,
  }),
  props: {},
  computed: {
    active_tab() {
      return data.order_report_tab_sub;
    },
  },
  watch: {
    active_tab() {
    ...
    active_tab_in() {
      data.order_report_tab_sub = this.active_tab_in;
    },
  },
  methods: {
    hideTabs() {
