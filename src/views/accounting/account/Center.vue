<template>
	<v-row class="">
		<v-col class="py-0 pr-4" sm="4" cols="12">
			<v-card outlined dense color="grayBg" class="pa-3 no_border mb-md-4 mb-xs-0" height="123px">
				<v-row>
					<v-col
						cols="12"
						sm="12"
			...
			<v-card outlined dense color="grayBg" class="pa-3 no_border hidden-sm-and-down">
				...
				<kendo-grid id="gridCOA" ref="gridCOA" class="grid-center center_heigth"
		...
		<v-col class="py-0 paddingLeft" sm="8" cols="12" style="margin-top: -12px;position:relative;">
			<v-row>
				<v-col sm="12" cols="12" class="tab_txn_att pl-0">
					<v-tabs >
						<v-tab>
						...
						<v-tab-item>
							<v-card flat color="grayBg" class="pa-3 custom_vcard">
								<v-card-text class="pt-0 bg_white">
						...
						<v-tab-item>
							<v-card flat class="" color="grayBg pa-3 ">
								<v-card-text class="bg_white">
									<Attachment :initAccount="account"/>
								</v-card-text>
							</v-card>
						</v-tab-item>
			...
			async onAccountTypeChanges() {
				new Promise(resolve => {
					...
					if (this.accountType !== null) {
						ds.filter( { field: "type_code", operator: "eq", value: this.accountType.code });
					} else {
						ds.filter([]);
					}
				})
				...
				new Promise(resolve => {
					...
					if (this.searchText !== "") {
						ds.filter({
							logic: "or",
							filters: [
								{ field: "number", operator: "contains", value: this.searchText },
...
<style scoped>
	...
	.custom_grid.k-grid tr*{ cursor: pointer;}
