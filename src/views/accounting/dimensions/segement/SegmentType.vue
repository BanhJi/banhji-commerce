			...
			<v-card color="white" class="pa-3 no_border" elevation="0">
				<v-row>
					<v-col sm="12" cols="12" class="pt-0">
						...
						<p class="mb-0">{{ $t("segment_setting_desc") }}</p>
					</v-col>
					<v-col sm="12" cols="12">
					<template>
						<div v-if="!isLoaded">
...
const segmentTypeModel = new SegmentTypeModel({});
...
export default {
	...
	methods: {
		...
		goEdit(e) {
			e.preventDefault();
			let grid = kendo.jQuery("#gridSegmentType").data("kendoGrid");
			let dataItem = grid.dataItem($(e.currentTarget).closest("tr"));
			this.segment = dataItem;
			this.mSegment = new SegmentTypeModel(this.segment.subOf);
			this.dialogM2 = true;
		},
		rowNumberTmpl(dataItem) {
			let ds = this.$refs.segmentTypeDS.kendoWidget(),
