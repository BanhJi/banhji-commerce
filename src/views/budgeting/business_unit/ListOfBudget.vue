...
export default {
	...
	methods: {
		...
		async search() {
			...
			budgets.forEach((value) => {
				if (value.location_uuid !== "" && value.segment_uuid === "") {
					let budget = new BudgetModel(value);
					budget.location = new LocationModel(
						locations.find((i) => i.id === value.location_uuid)
					);
			...
			if (this.location !== null) {
				budgetList = budgetList.filter(
					(value) => value.location_uuid === this.location.id
				);
			}
	...
	watch: {
		$route(to, from) {
			if (from.name === "Business Unit Budget") {
				this.search();
			}
		},
	},
