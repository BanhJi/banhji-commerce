...
export default {
  ...
  methods: {
        ...
        setTimeout(() => {
          ...
          accountHandler.getAll().then((res) => {
            ...
            if (this.accRevenue.length > 0) {
              this.mAccRevenue = this.accRevenue[1];
            }
            ...
            if (this.accInventory.length > 0) {
              this.mAccInventory = this.accInventory[1];
            }
            ...
            if (this.accCostOfGoodSold.length > 0) {
              this.mAccCostOfGoodSold = this.accCostOfGoodSold[1];
            }
    ...
    async loadPage(that) {
      $(".category-list").html(`
                <span class="mt-2 prod-category-list-item-returned">0</span><br/>
                <div style="display: flex; flex-direction: 'row'">
                    <button class="prod-category-list-next-page" title="previous" style="width: 24px; font-size: 19px; margin-top: -6px;"> < </button>
                    <ul class="prod-category-list-page-number page_scroll" style="display: flex;list-style: none; padding-left: 0px;">
                        <li style="padding: 6px; padding-bottom: 0px;"  data-page="1" class="prod-category-list-page-link text-bold-link" >1</li>
                    </ul>
                    <button class="prod-category-list-next-page" title="next" style="width: 24px; font-size: 19px;margin-top: -6px;"> > </button>
                </div>
            `);
      $(document).on("click", ".prod-category-list-page-link", function () {
        const page = $(this).data("page");
        $(".prod-category-list-page-link").removeClass("text-bold-link");
        $(this).addClass("text-bold-link");
        that.backToPage(that, page - 1);
        window.console.log(page - 1);
      });
      // $(document).on('click', '.bycus-next-page', function () {
      $(".prod-category-list-next-page").click(function () {
        const page_number = $(this)
          .parent()
          .children(".prod-category-list-page-number")
          .find("li")
          .last()
          .data("page");
        window.console.log("prod-category-list-page_number", page_number);
        ...
        if (isPrevious == "previous") {
          ...
          $(".prod-category-list-page-link").removeClass("text-bold-link");
          $(`.prod-category-list-page-link:eq(${currectpage - 1})`).addClass(
            "text-bold-link"
          );
          ...
          $(".prod-category-list-page-link").removeClass("text-bold-link");
          that.loadProductCategory(that.search);
          $(this)
            .parent()
            .children(".prod-category-list-page-number")
            .append(
              `<li class="prod-category-list-page-link text-bold-link" style="padding: 6px; padding-bottom: 0px;" data-page="${
                page_number + 1
              }">${page_number + 1}</li>`
            );
        }
    ...
    itemRetuned(that) {
      let totalRow = 0;
      that.txnCategoryList.forEach((k) => {
        totalRow += k.length;
      });
      $(".prod-category-list-item-returned").text(
        "Items returned (" + totalRow + ")"
      );
    },
    disabledNextPage(value) {
      $(".prod-category-list-next-page").prop("disabled", value);
    },
    backToPage(that, index) {
      try {
        const byProduct = that.txnCategoryList.at(index);
        that.categoryList = byProduct;
        // that.refreshGridData(customer)
        window.console.log("that.txnCategoryList[index]", byProduct);
      } catch (e) {
        window.console.log("ERROR on backToPage", index);
      }
    },
    onSearch(search) {
      this.txnCategoryList = [];
      this.loadPage(this);
      this.paging = {};
      this.loadProductCategory(search);
    },
        ...
        setTimeout(() => {
          ...
          categoryHandler.getv2(this.params).then((res) => {
            if (res.data.statusCode === 200) {
              ...
              this.txnCategoryList.push(responseData);

              this.toalCategoryPerPage = this.categoryList.length;
