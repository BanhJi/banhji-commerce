            ...
            <v-row>
              <v-col class="bigSide py-0" sm="8" cols="12" style="transition: .3s ease-in;"
                ...
                <v-form ref="form" v-model="valid" lazy-validation>
                  <v-card outlined dense class="no_border">
                    <h2 class="mb-0">{{ $t('inventory_adjustment') }} #{{ inventoryAdjustment.referenceNo }}</h2>
                  ...
                  <v-card outlined dense class="no_border function_footer mt-0 py-2">
                    <p class="float-left  color_grey py-3 mb-0">{{ $t('standard_inventory_adjustment') }}</p>
                    <!--                    <v-btn color="lightBlue" class="white&#45;&#45;text float-right  mx-1 text-capitalize">-->
                    <!--                      {{ $t('print') }}-->
                    <!--                    </v-btn>-->
                    <!--                    <v-btn color="primary" class="white&#45;&#45;text float-right mx-1 text-capitalize">-->
                    <!--                      {{ $t('delete') }}-->
                    <!--                    </v-btn>-->
                    <v-btn color="primary" class="white--text float-right mx-1 text-capitalize" @click="onEdit('edit')">
                      {{ $t('edit') }}
                    </v-btn>
                  </v-card>
                  <v-divider/>
              ...
              <v-col class="smallSide" sm="4" style="transition: .3s ease-in;"
                ...
                <div v-if="!isHideBar" class="small_sidebar mt-9 grayBg">
                  ...
                  <p class="mb-0  font_14">{{ $t('inventory_adjustment_func_desc') }}</p>
                  <div class="my-2 mt-3" style="height:3px; background-color:#ffffff;width:100%;"></div>
                  <p class="color_grey mt-4 mb-2">{{ $t('transaction_history') }}</p>
                  <v-simple-table>
    ...
    async loadViewAdjustment() {
      new Promise(resolve => {
        setTimeout(() => {
          resolve('resolved')
          this.showLoading = true
          billingHandler.inventoryAdjustmentList('?id=' + this.$route.params.id).then(res => {
            if (res.data.statusCode === 200) {
              this.showLoading = false
    ...
    transactionDate() {
      if (this.inventoryAdjustment) {
        if (this.inventoryAdjustment.hasOwnProperty('transactionDate')) {
          return kendo.toString(new Date(this.inventoryAdjustment.transactionDate), this.dateFormat)
        }
      }
      return ''
    },
      ...
      if (this.inventoryAdjustment) {
        if (this.inventoryAdjustment.hasOwnProperty('employee')) {
          if (this.inventoryAdjustment.employee.hasOwnProperty('id')) {
            return this.inventoryAdjustment.employee.firstName + ' - ' + this.inventoryAdjustment.employee.lastName
          }
        }
      }
      return ''
