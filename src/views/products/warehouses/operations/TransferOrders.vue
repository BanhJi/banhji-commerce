        ...
        <v-col sm="12" cols="12" class="pt-0">
            <v-row class="">
                <v-col sm="5" cols="12" class="pt-0">
                    <h2 class="mb-0 font_20">{{ $t('transfer_orders') }}</h2>
                </v-col>
                <v-col sm="7" cols="12">
<!--                    <v-row class="">-->
...
<!--                                        <p class="white&#45;&#45;text mb-0 text-uppercase">{{ $t('incoming_transfer') }}</p>-->
<!--                                    </v-col>-->
<!--                                    <v-col class="py-1" sm="6" cols="12">-->
...
<!--                                <v-row class="">-->
<!--                                    <v-col class="py-1" sm="6" cols="12">-->
<!--                                        <p class="white&#45;&#45;text mb-0 text-uppercase">{{ $t('ready_for_transfer') }}</p>-->
<!--                                    </v-col>-->
<!--                                    <v-col class="py-1" sm="6" cols="12">-->
<!--                                        <h3 class="white&#45;&#45;text mt-3 text-right">100</h3>-->
<!--                                    </v-col>-->
...
<!--                    </v-row>-->
            ...
            <v-row class="">
                ...
                <v-col sm="3" cols="12" class="pt-1">
                    <v-btn to="transfer_order" color="primary" class=" white--text float-right text-capitalize">
                        {{$t('new_transfer')}}
                    </v-btn>
                </v-col>
            </v-row>
                    ...
                    <template>
                        <v-simple-table class="attachment_table">
                            <template v-slot:default>
                                ...
                                <tbody>
                                <tr v-for="to in transferOrders" v-bind:key="to.id">
                                    <td>{{ new Date(to.date).toISOString().substr(0, 10) }}</td>
                                    <td>{{ to.number }}</td>
                                    <td>{{ to.fWarehouse ? to.fWarehouse.name : '' }}</td>
                                    <td>{{ to.tWarehouse ? to.tWarehouse.name : '' }}</td>
                                    <td></td>
...
export default {
    ...
    props: {},
    methods: {
        async onloadTransferOrder() {
            await transferOrderHandler.getAll().then(res => {
        ...
        searchTransaction() {
            ...
            transferOrderHandler.getAll(data).then(res => {
                this.transferOrders = res
                window.console.log(res)
            })
        },
    ...
    async activated() {
        await this.onloadTransferOrder()
    },
    async mounted() {
        await this.onloadTransferOrder()
    },
