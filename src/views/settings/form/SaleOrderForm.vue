<template>
  <v-row class="">
    <v-col sm="12" cols="12">
      <v-row>
        <v-col style="padding-right: 109px !important;" sm="12" cols="12" class="pt-0">
          <div class="a4">
          <div style="height: 42px;" class="mb-2">
               <v-btn class="primary text-capitalize float-right" @click="EdtiForm(1)">
                {{ $t("edit") }}
              </v-btn>
              <v-btn class="secondary text-capitalize float-right mx-3" @click="_print(1)">{{
                $t("preview")
              }}</v-btn>
              <v-btn
                class=" capitalize text-capitalize float-right mx-0"
                color="primary"
                @click="ResetForm"
                >{{ $t("reset") }}</v-btn
              >  
...
import { SaleOrder, print } from "@/form/Sale.js";
const {
  getFormSetting,
  formSettingCreate,
  deleteFormSetting,
...
let def = {
  title: {
    en: { name: "Sale Order", status: true },
    kh: { name: "ការបញ្ជាទិញលក់", status: true },
  },
  page: "A4",
  ...
  signature: {
    prepared_by: { status: true, text: "Prepare By", text_kh: "" },
    reviewed_by: {
      status: true,
      text: "Reviewed By",
      text_kh: "",
    },
    verified_by: { status: true, text: "Approverd By", text_kh: "" },
    approved_by: { status: true, text: "Recorded By", text_kh: "" },
    recorded_by: { status: true, text: "Received By", text_kh: "" },
    option: 1,
    positon: true,
  },
  language: "English",
...
export default {
  ...
  data() {
    return {
      ...
      pk: "",
      invoice_setting: def,
      form_invoice: SaleOrder(this.items, "id_23e", def),
      slider_widht: 5,
      slider_height: 5,
      image_size: {
        width: "100px",
      ...
      if (this.pk != "") {
        deleteFormSetting(params).then((res) => {
          ...
          if (res.data.statusCode === 200) {
            ...
            this.form_invoice = SaleOrder(this.items, this.invoice_setting);
          } else {
