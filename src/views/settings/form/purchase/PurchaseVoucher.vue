...
import { PurchaseVoucher, print } from "@/form/Purchases.js";
const {
  getFormSetting,
  formSettingCreate,
  deleteFormSetting,
...
let def = {
  title: {
    en: { name: "Purchase Voucher", status: true },
    kh: { name: "ទិញប័ណ្ណទូទាត់", status: true },
  },
...
export default {
  ...
  data() {
    return {
      ...
      loading: false,
      invoice_setting: def,
      form_invoice: PurchaseVoucher(this.items, def),
      slider_widht: 5,
      slider_height: 5,
      image_size: {
        width: "100px",
  ...
  methods: {
      ...
      if (this.pk != "") {
        deleteFormSetting(params).then((res) => {
          ...
          if (res.data.statusCode === 200) {
            ...
            this.form_invoice = PurchaseVoucher(
              this.items,
    ...
    UpdateFormChange() {
      this.form_invoice = PurchaseVoucher(this.items, this.invoice_setting);
    },
    logoPosition(pos) {
