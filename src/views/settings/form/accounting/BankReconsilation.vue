...
import { BankReconciliation, print } from "@/form/Accounting.js";
const {
  getFormSetting,
  formSettingCreate,
  deleteFormSetting,
...
let def = {
  title: {
    en: { name: "Bank Reconciliation", status: true },
  ...
  signature: {
    ...
    approved_by: { status: false, text: "Recorded By", text_kh: "ទូទាត់ដោយ" },
    recorded_by: { status: false, text: "Received By", text_kh: "ទទួលដោយ" },
    option: 2,
    positon: true,
  },
  language: "English",
...
export default {
  ...
  data() {
    return {
      ...
      loading: false,
      invoice_setting: def,
      form_invoice: BankReconciliation(this.items, def),
      slider_widht: 5,
      slider_height: 5,
      image_size: {
        width: "100px",
  ...
  methods: {
      ...
      if (this.pk != "") {
        deleteFormSetting(params).then((res) => {
          ...
          if (res.data.statusCode === 200) {
            ...
            this.form_invoice = BankReconciliation(
              this.items,
    ...
    UpdateFormChange() {
      this.form_invoice = BankReconciliation(this.items, this.invoice_setting);
    },
    logoPosition(pos) {
