...
import { JournalVourcher, print } from "@/form/Accounting.js";
const {
  getFormSetting,
  formSettingCreate,
  deleteFormSetting,
...
let def = {
  title: {
    en: { name: "Journal Voucher", status: true },
    kh: { name: "សក្ខីប័ត្រនិទ្មានុប្បវត្តិ", status: true },
  },
...
export default {
  ...
  data() {
    return {
      ...
      loading: false,
      invoice_setting: def,
      form_invoice: JournalVourcher(this.items, def),
      slider_widht: 5,
      slider_height: 5,
      image_size: {
        width: "100px",
  ...
  methods: {
      ...
      if (this.pk != "") {
        deleteFormSetting(params).then((res) => {
          ...
          if (res.data.statusCode === 200) {
            ...
            this.form_invoice = JournalVourcher(
              this.items,
    ...
    UpdateFormChange() {
      this.form_invoice = JournalVourcher(this.items, this.invoice_setting);
    },
    logoPosition(pos) {
