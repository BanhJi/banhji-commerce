...
import { BankTransaction, print } from "@/form/Accounting.js";
const {
  getFormSetting,
  formSettingCreate,
  deleteFormSetting,
...
let def = {
  title: {
    en: { name: "Cash Transaction", status: true },
    kh: { name: "ប្រតិបត្តិការសាច់ប្រាក់", status: true },
  },
...
export default {
  ...
  data() {
    return {
      ...
      loading: false,
      invoice_setting: def,
      form_invoice: BankTransaction(this.items, def),
      slider_widht: 5,
      slider_height: 5,
      image_size: {
        width: "100px",
  ...
  methods: {
      ...
      if (this.pk != "") {
        deleteFormSetting(params).then((res) => {
          ...
          if (res.data.statusCode === 200) {
            ...
            this.form_invoice = BankTransaction(
              this.items,
    ...
    UpdateFormChange() {
      this.form_invoice = BankTransaction(this.items, this.invoice_setting);
    },
    logoPosition(pos) {
