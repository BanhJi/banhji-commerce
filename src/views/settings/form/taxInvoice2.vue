      ...
      <v-row>
        <v-col sm="12" cols="12" class="pt-0">
         ...
         <div style="height: 42px;" class="mb-2">
              <v-btn color="third" class="white--text text-capitalize float-right" @click="EdtiForm(1)">
                {{ $t("edit") }}
              </v-btn>
              <v-btn
                color="third"
                class=" white--text  text-capitalize float-right mx-3"
                @click="_print(4)"
                >{{ $t("preview") }}</v-btn
              >
              <v-btn
...
import { taxInvoice2, print } from "@/form/invoices.js";
...
export default {
  ...
  data() {
    return {
      ...
      form_invoice: taxInvoice2(this.items, "2", def),
      slider_widht: 5,
      slider_height: 5,
      image_size: {
        width: "100px",
  ...
  methods: {
      ...
      if (this.pk != "") {
        deleteFormSetting(params).then(async (res) => {
          if (res.data.statusCode === 200) {
            ...
            this.form_invoice = await taxInvoice2(
              this.items,
    ...
    getFormSettingS() {
      let params = "?formType=Tax Invoice2";
