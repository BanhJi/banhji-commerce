...
import { AdvanaceSettlement, print } from "@/form/Expense.js";
const {
  getFormSetting,
  formSettingCreate,
  deleteFormSetting,
...
let def = {
  title: {
    en: { name: "Advacne Settlement Voucher", status: true },
    kh: { name: "សក្ខីប័ត្រជំរះបុរេប្រទាន", status: true },
  },
...
export default {
  ...
  data() {
    return {
      ...
      loading: false,
      invoice_setting: def,
      form_invoice: AdvanaceSettlement(this.items, def),
      slider_widht: 5,
      slider_height: 5,
      image_size: {
        width: "100px",
  ...
  methods: {
      ...
      if (this.pk != "") {
        deleteFormSetting(params).then((res) => {
          ...
          if (res.data.statusCode === 200) {
            ...
            this.form_invoice = AdvanaceSettlement(
              this.items,
    ...
    saveForm() {
      ...
      let data = {
        type: "form",
        formType: "AdvanaceSettlement Voucher",
    ...
    UpdateFormChange() {
      this.form_invoice = AdvanaceSettlement(this.items, this.invoice_setting);
    },
    logoPosition(pos) {
