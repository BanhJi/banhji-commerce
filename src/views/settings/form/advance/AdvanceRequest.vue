...
import { AdvanceRequest, print } from "@/form/Expense.js";
const {
  getFormSetting,
  formSettingCreate,
  deleteFormSetting,
...
let def = {
  title: {
    en: { name: "Advance Request", status: true },
    kh: { name: "សំណើរបុរេប្រទាន", status: true },
  },
...
export default {
  ...
  data() {
    return {
      ...
      loading: false,
      invoice_setting: def,
      form_invoice: AdvanceRequest(this.items, def),
      slider_widht: 5,
      slider_height: 5,
  ...
  methods: {
      ...
      if (this.pk != "") {
        deleteFormSetting(params).then((res) => {
          ...
          if (res.data.statusCode === 200) {
            ...
            this.form_invoice = AdvanceRequest(
              this.items,
    ...
    UpdateFormChange() {
      this.form_invoice = AdvanceRequest(this.items, this.invoice_setting);
    },
    logoPosition(pos) {
