            ...
            <v-row>
              <v-col sm="3" cols="12" class="hide_msg">
                ...
                <v-row>
                  ...
                  <!-- color -->
                  <v-divider />
                  <label class="ml-2">{{ $t("color_background") }}</label>
...
import { AdvanceVourcher, print } from "@/form/Expense.js";
const {
  getFormSetting,
  formSettingCreate,
  deleteFormSetting,
...
let def = {
  title: {
    en: { name: "Advance Voucher", status: true },
    kh: { name: "សក្ខីប័ត្របុរេប្រទាន", status: true },
  },
  logo: { width: 140, height: 140, position: "left", hide: false, isPrimrar: false },
  header: false,
  page: "A4",
  form_number: 1,
  table_header: 1,
  ...
  signature: {
    ...
    posted_by: { status: false, text: "Posted By", text_kh: "ប្រកាសដោយ" },
...
export default {
  ...
  data() {
    return {
      ...
      loading: false,
      invoice_setting: def,
      form_invoice: AdvanceVourcher(this.items, def),
      slider_widht: 5,
      slider_height: 5,
      image_size: {
        width: "100px",
  ...
  methods: {
      ...
      if (this.pk != "") {
        deleteFormSetting(params).then((res) => {
          ...
          if (res.data.statusCode === 200) {
            ...
            this.form_invoice = AdvanceVourcher(
              this.items,
    ...
    UpdateFormChange() {
      this.form_invoice = AdvanceVourcher(this.items, this.invoice_setting);
    },
    logoPosition(pos) {
