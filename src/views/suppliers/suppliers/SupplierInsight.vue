            ...
            <v-card outlined dense color="white" class="no_border mb-4">
              <v-row>
                <v-col sm="6" cols="12" class="pt-0">
                  <h3 class="text-left mb-0 font_20">{{ $t('vendors_balance_payments') }}</h3>
                </v-col>
              </v-row>
              <chart ref="chart"
        ...
        <v-row>
          <v-col class="py-0 pr-md-2" sm="4" cols="12">
            ...
            <v-card outlined dense color="third"
              ...
              <v-row>
                <h4 class="py-0 text-uppercase col-sm-12 col-md-7 font_13">
                  {{ $t('vendors_with_overdue_invoices') }}</h4>
          ...
          <v-col class="py-0 pl-md-2" sm="4" cols="12">
            <v-card
                ...
                color="grayBg"
            >
              <h3 class="font_20">{{ $t("total_vendors") }}</h3>
              <p class="mb-0">
                {{ $t('as_of_today') }}
              </p>
              <h2 class="primary--text mb-0 pa-0 niradei_black mb-0 col-sm-12 text-right">
        ...
        setTimeout(() => {
          ...
          billingHandler.wcpPableOverviewList(strFilter).then(res => {
            if (res.data.statusCode === 200) {
              ...
              this.amountToPayBalance = this.numberFormat(response.openBalance, response.decimalFormat)
    ...
    async dashboardReceivableOverviewVendor() {
      new Promise(resolve => {
        setTimeout(() => {
          ...
          billingHandler.dashboardReceivableOverviewVendor(strFilter).then(res => {
            if (res.data.statusCode === 200) {
